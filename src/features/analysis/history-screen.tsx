...
export function HistoryScreen({ initialAnalysisId }: { initialAnalysisId?: string }) {
  ...
  return <div className="history-screen">
    ...
    {loading ? <section className="panel empty-state"><h2>Loading history…</h2></section> : error ? <section className="panel empty-state"><h2>History unavailable</h2><p role="alert" className="negative">{error}</p></section> : analyses.length === 0 ? <section className="panel empty-state"><h2>No analyses yet</h2><p>Run your first card through the manual evidence workflow. The immutable snapshot will appear here.</p><Link className="primary-button" href="/">Analyze a card</Link></section> : <>
